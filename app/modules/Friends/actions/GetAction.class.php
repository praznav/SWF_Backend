...
class Friends_GetAction extends ShoppingwithfriendsFriendsBaseAction
{
	/**
    ...
    public function executeRead(AgaviRequestDataHolder $rd)
    {
        ...
        if ($authenticationStatus == 0)
        {
            ...
            $friend = $friendManager->get($username, $friendUsername);
            if (isset($friend) && $friend instanceof Friends_FriendModel)
            {
                $this->setAttribute('friend', $friend);
                return 'Success';
            }
            else if ($friend == 1)
            {
